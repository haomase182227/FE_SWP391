...
const mockBikes = [
  { id: 1, name: 'Trek Domane SL 6', seller: 'Nguyen Van A', location: 'Hanoi', year: 2022, price: '$1,200', image: null },
  { id: 2, name: 'Giant Defy Advanced', seller: 'Tran Thi B', location: 'HCM City', year: 2021, price: '$980', image: null },
  { id: 3, name: 'Specialized Roubaix', seller: 'Le Van C', location: 'Da Nang', year: 2023, price: '$1,500', image: null },
  { id: 4, name: 'Cannondale Synapse', seller: 'Pham Thi D', location: 'Hanoi', year: 2020, price: '$850', image: null },
  { id: 5, name: 'Scott Addict RC', seller: 'Hoang Van E', location: 'HCM City', year: 2022, price: '$2,100', image: null },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-surface-container-lowest rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        ...
        <div className="px-6 py-6 space-y-3">
          <p className="font-label uppercase text-[10px] tracking-widest text-on-surface-variant mb-4">
            Tích vào các mục đã kiểm tra
          </p>
          {CHECKLIST.map(item => (
            <button
              key={item.id}
            ...
            >
              <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${
                checked[item.id] ? 'bg-tertiary' : 'bg-surface-container-high'
              }`}>
              ...
              <div className="flex-1">
                <p className={`font-headline font-bold text-sm ${checked[item.id] ? 'text-tertiary-dim' : 'text-on-surface'}`}>
                  {item.label}
                </p>
                <p className="text-[11px] text-on-surface-variant mt-0.5">{item.desc}</p>
              </div>
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                checked[item.id] ? 'bg-tertiary border-tertiary' : 'border-outline'
              }`}>
                {checked[item.id] && (
                  <span className="material-symbols-outlined text-on-tertiary" style={{ fontSize: '14px' }}>check</span>
                )}
              </div>
            </button>
          ))}
        ...
        <div className="px-6 py-5 flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-3 rounded-xl border-2 border-outline text-on-surface-variant font-bold uppercase text-[11px] tracking-widest hover:bg-surface-container-low transition-colors"
          >
            Huỷ
          </button>
          <button
            onClick={handleInspect}
            className="flex-1 py-3 rounded-xl font-bold uppercase text-[11px] tracking-widest transition-all bg-primary text-on-primary hover:opacity-90"
          >
...
export default function InspectorManagement() {
  // status: null | 'accepted' | 'Inspected' | 'Declined'
  const [bikeStatus, setBikeStatus] = useState({});
  const [inspecting, setInspecting] = useState(null); // bike being inspected

  function handleAccept(id) {
    setBikeStatus(prev => ({ ...prev, [id]: 'accepted' }));
  }

  function handleStart(bike) {
    setInspecting(bike);
  }

  function handleResult(id, result) {
    setBikeStatus(prev => ({ ...prev, [id]: result }));
  }
    ...
    <div className="bg-surface font-body text-on-surface antialiased min-h-screen">
      ...
      <aside className="fixed left-0 h-full w-64 z-50 bg-zinc-50 flex flex-col py-8 px-4 border-r border-zinc-200/20">
        ...
        <nav className="flex-1 space-y-1">
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-orange-50 text-primary font-bold border-r-4 border-primary">
            <span className="material-symbols-outlined text-primary" style={{ fontVariationSettings: '"FILL" 1' }}>fact_check</span>
            <span className="font-body uppercase tracking-widest text-[10px] font-medium">Inspection Queue</span>
          </div>
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-600 hover:bg-zinc-100 cursor-pointer">
            <span className="material-symbols-outlined">history</span>
            <span className="font-body uppercase tracking-widest text-[10px] font-medium">History</span>
          </div>
          <div className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-zinc-600 hover:bg-zinc-100 cursor-pointer">
            <span className="material-symbols-outlined">person</span>
            <span className="font-body uppercase tracking-widest text-[10px] font-medium">My Profile</span>
          </div>
        </nav>
        <div className="mt-auto pt-4 border-t border-zinc-200/20">
          <div className="flex items-center gap-3 px-3 py-2 text-zinc-600 hover:text-error hover:bg-error-container/10 transition-all rounded-lg cursor-pointer">
      ...
      <main className="ml-64 pt-24 px-8 pb-12 min-h-screen">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            <span className="label-md uppercase font-bold text-on-surface-variant tracking-widest mb-2 block text-xs">
              Inspector Workspace
            </span>
                ...
                return (
                  <tr key={bike.id} className="hover:bg-primary-container/5 transition-colors">
                    ...
                    <td className="px-8 py-5 text-right">
                      <ActionButton
                        status={status}
                        onAccept={() => handleAccept(bike.id)}
                        onStart={() => handleStart(bike)}
                      />
                    </td>
                  </tr>
                );
...
function StatusBadge({ status }) {
  ...
  if (status === 'Declined') return (
    <span className="px-3 py-1 bg-error/10 text-error rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 w-fit">
      <span className="material-symbols-outlined text-[12px]" style={{ fontVariationSettings: '"FILL" 1' }}>cancel</span>
      Declined
    </span>
  );
}
...
function ActionButton({ status, onAccept, onStart }) {
  if (!status) return (
    <button
      onClick={onAccept}
      className="px-5 py-2 bg-secondary text-on-secondary hover:opacity-90 transition-opacity uppercase font-bold text-[10px] rounded-lg tracking-widest"
    >
      Accept
    </button>
  );
  if (status === 'accepted') return (
    <button
      onClick={onStart}
      className="px-5 py-2 bg-primary text-on-primary hover:opacity-90 transition-opacity uppercase font-bold text-[10px] rounded-lg tracking-widest flex items-center gap-1.5"
    >
  ...
  return (
    <button disabled className="px-5 py-2 bg-surface-container-high text-on-surface-variant uppercase font-bold text-[10px] rounded-lg tracking-widest cursor-not-allowed opacity-50">
      Done
    </button>
  );
}
